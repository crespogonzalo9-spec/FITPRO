      ...
      {visibleRoutines.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {visibleRoutines.map(routine => (
            <Card key={routine.id} className="cursor-pointer hover:border-gray-600 transition-colors" onClick={() => { setSelected(routine); setShowView(true); }}>
...
const RoutineModal = ({ isOpen, onClose, onSave, routine, classes, members, exercises }) => {
  ...
  const addExercise = () => {
    setForm(prev => ({ 
      ...prev, 
      exercises: [...prev.exercises, { exerciseId: '', sets: 3, reps: '10', rest: '60', notes: '' }] 
    }));
  };
...
const ViewRoutineModal = ({ isOpen, onClose, routine, exercises, getClassName, getMemberNames, members }) => {
