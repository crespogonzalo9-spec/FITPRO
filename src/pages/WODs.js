...
import { Plus, Flame, MoreVertical, Edit, Trash2, Users, User, Calendar, Clock, Eye, Lock } from 'lucide-react';
...
const WODs = () => {
  ...
  useEffect(() => {
    ...
    if (isAlumno()) {
      ...
      return () => { unsubWods(); unsubClasses(); unsubEnrollments(); };
    }

    return () => { unsubWods(); unsubClasses(); };
  }, [currentGym, userData, isAlumno]);

  // Filtrar WODs visibles para alumnos
  const visibleWods = isAlumno() 
    ? wods.filter(w => {
        if (!w.assignmentType || w.assignmentType === 'class') {
          return !w.classId || myEnrollments.includes(w.classId);
        }
        if (w.assignmentType === 'individual') {
          return w.memberIds?.includes(userData.id);
        }
        return true;
      })
    : wods;

  const filtered = visibleWods.filter(w => w.name?.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400">{filtered.length} workouts</p>
        </div>
        {canCreateRoutines() && (
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {filtered.map(wod => (
            <Card key={wod.id} className="hover:border-primary-30 cursor-pointer" onClick={() => { setSelected(wod); setShowView(true); }}>
                    ...
                    <div className="flex gap-2 mt-1">
                      ...
                      {wod.timeLimit && (
                        <Badge className="bg-gray-500/20 text-gray-400">
                          <Clock size={12} className="mr-1" />{wod.timeLimit}'
                        </Badge>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex justify-between items-center text-sm text-gray-500">
                <span>
                  {wod.assignmentType === 'class' && wod.classId ? (
                    <span className="flex items-center gap-1"><Calendar size={14} /> {getClassName(wod.classId)}</span>
                  ) : wod.assignmentType === 'individual' && wod.memberIds?.length > 0 ? (
                    <span className="flex items-center gap-1"><Lock size={14} /> {wod.memberIds.length} alumnos</span>
                  ) : (
                    <span>General</span>
                  )}
                </span>
                <span>{formatDate(wod.createdAt)}</span>
              </div>
            </Card>
          ))}
        </div>
...
const WODModal = ({ isOpen, onClose, onSave, wod, classes, members }) => {
  ...
  useEffect(() => {
    if (wod) {
      ...
      setForm({ name: '', type: 'for_time', description: '', timeLimit: '', assignmentType: 'class', classId: '', memberIds: [] });
    }
  }, [wod, isOpen]);
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={wod ? 'Editar WOD' : 'Nuevo WOD'} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex gap-2">
          ...
          <div className="pt-6">
            <Button type="button" variant="secondary" size="sm" onClick={() => setShowBenchmarks(!showBenchmarks)}>
              Benchmarks
            </Button>
          </div>
        </div>
        ...
        <Textarea label="Descripción / Movimientos" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={5} placeholder="21-15-9&#10;Thrusters (95/65 lb)&#10;Pull-ups" />

        <Select label="Asignar a" value={form.assignmentType} onChange={e => setForm({ ...form, assignmentType: e.target.value, classId: '', memberIds: [] })} options={[{ value: 'class', label: 'Clase o General' }, { value: 'individual', label: 'Alumnos específicos' }]} />

        {form.assignmentType === 'class' ? (
          <Select label="Clase (opcional - dejar vacío para general)" value={form.classId} onChange={e => setForm({ ...form, classId: e.target.value })} options={[{ value: '', label: 'General (todos)' }, ...classes.map(c => ({ value: c.id, label: `${c.name} - ${c.startTime}` }))]} />
        ) : (
          <div>
            ...
            <div className="max-h-40 overflow-y-auto space-y-2 p-3 bg-gray-800/50 rounded-xl">
              {members.map(m => (
                <label key={m.id} className="flex items-center gap-3 p-2 hover:bg-gray-700 rounded-lg cursor-pointer">
                  <input type="checkbox" checked={form.memberIds.includes(m.id)} onChange={() => toggleMember(m.id)} className="w-4 h-4 rounded" />
